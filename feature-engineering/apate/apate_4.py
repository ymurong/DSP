...
class ApateGraphFeatures:
    ...
    def __init__(self, training_data, convergence_threshold=0.0000001, alpha=0.85):
        ...
        for row in training_data.itertuples():
            ...
            '''
            rows_extend(
                [t, t, t, c + num_t, ip + num_c + num_t, e + num_c + num_t + num_ip]
            )
            cols_extend(
                [c + num_t, ip + num_c + num_t, e + num_c + num_t + num_ip, t, t, t]
            )
            A_extend(
                [1.0] * 6
    ...
    def compute_trx_score(self, row, interval):
        ...
        if card < self.num_c and ip < self.num_ip and email < self.num_e:
            ...
            for nonzero_col in reversed(nonzero_cols):
                ...
                # third for ip
                # forth for email
                if len(nonzero_rows_2) != 3:
                    print("ERROR: len(nonzero_rows_2) != 3")

                old_ip_idx = nonzero_rows_2[1] - self.num_t - self.num_c
                old_email_idx = nonzero_rows_2[2] - self.num_t - self.num_c - self.num_ip

                if old_email_idx == email and old_ip_idx == ip:
        ...
        if ip < self.num_ip:
            ip_col = self.num_t + self.num_c + ip
            ...
            if interval == self.NO_INTERVAL:
                if self.A_tri_avg_ip is None:
                    ...
                    for p in range(self.num_ip):
                        total += self.A_tri.getcol(self.num_t + self.num_c + p).sum()
                ...
                if self.A_tri_short_avg_ip is None:
                    ...
                    total = 0
                    for p in range(self.num_ip):
                        total += self.A_tri_short.getcol(self.num_t + self.num_c + p).sum()
                ...
                if self.A_tri_medium_avg_ip is None:
                    ...
                    total = 0
                    for p in range(self.num_ip):
                        total += self.A_tri_medium.getcol(self.num_t + self.num_c + p).sum()
                ...
                if self.A_tri_long_avg_ip is None:
                    ...
                    total = 0
                    for p in range(self.num_ip):
                        total += self.A_tri_long.getcol(self.num_t + self.num_c + p).sum()
        ...
        if email < self.num_e:
            email_col = self.num_t + self.num_c + self.num_ip + email
            ...
            if interval == self.NO_INTERVAL:
                if self.A_tri_avg_email is None:
                    ...
                    for e in range(self.num_e):
                        total += self.A_tri.getcol(self.num_t + self.num_c + self.num_ip + e).sum()
                ...
                if self.A_tri_short_avg_email is None:
                    ...
                    total = 0
                    for e in range(self.num_e):
                        total += self.A_tri_short.getcol(self.num_t + self.num_c + self.num_ip + e).sum()
                ...
                if self.A_tri_medium_avg_email is None:
                    ...
                    total = 0
                    for e in range(self.num_e):
                        total += self.A_tri_medium.getcol(self.num_t + self.num_c + self.num_ip + e).sum()
                ...
                if self.A_tri_long_avg_email is None:
                    ...
                    total = 0
                    for e in range(self.num_e):
                        total += self.A_tri_long.getcol(self.num_t + self.num_c + self.num_ip + e).sum()
        ...
        return card_influence + ip_influence + email_influence
        ...
        if ip >= self.num_ip:
            ...
            if interval == self.NO_INTERVAL:
                if self.rkc_avg_ip is None:
                    ...
                    total = 0
                    for p in range(self.num_ip):
                        total += self.rkc[self.num_t + self.num_c + p, 0]
                ...
                if self.rkc_short_avg_ip is None:
                    ...
                    total = 0
                    for p in range(self.num_ip):
                        total += self.rkc_short[self.num_t + self.num_c + p, 0]
                ...
                if self.rkc_medium_avg_ip is None:
                    ...
                    total = 0
                    for p in range(self.num_ip):
                        total += self.rkc_medium[self.num_t + self.num_c + p, 0]
                ...
                if self.rkc_long_avg_ip is None:
                    ...
                    total = 0
                    for p in range(self.num_ip):
                        total += self.rkc_long[self.num_t + self.num_c + p, 0]
                    self.rkc_long_avg_ip = total / self.num_ip

                return self.rkc_long_avg_ip

        elif interval == self.NO_INTERVAL:
            return self.rkc[self.num_t + self.num_c + ip, 0]
        elif interval == self.SHORT_TERM:
            return self.rkc_short[self.num_t + self.num_c + ip, 0]
        elif interval == self.MEDIUM_TERM:
            return self.rkc_medium[self.num_t + self.num_c + ip, 0]
        elif interval == self.LONG_TERM:
            return self.rkc_long[self.num_t + self.num_c + ip, 0]
        ...
        if email >= self.num_e:
            ...
            if interval == self.NO_INTERVAL:
                if self.rkc_avg_email is None:
                    ...
                    total = 0
                    for e in range(self.num_e):
                        total += self.rkc[self.num_t + self.num_c + self.num_ip + e, 0]
                ...
                if self.rkc_short_avg_email is None:
                    ...
                    total = 0
                    for e in range(self.num_e):
                        total += self.rkc_short[self.num_t + self.num_c + self.num_ip + e, 0]
                ...
                if self.rkc_medium_avg_email is None:
                    ...
                    total = 0
                    for e in range(self.num_e):
                        total += self.rkc_medium[self.num_t + self.num_c + self.num_ip + e, 0]
                ...
                if self.rkc_long_avg_email is None:
                    ...
                    total = 0
                    for e in range(self.num_e):
                        total += self.rkc_long[self.num_t + self.num_c + self.num_ip + e, 0]
                    self.rkc_long_avg_email = total / self.num_e

                return self.rkc_long_avg_email

        elif interval == self.NO_INTERVAL:
            return self.rkc[self.num_t + self.num_c + self.num_ip + email, 0]
        elif interval == self.SHORT_TERM:
            return self.rkc_short[self.num_t + self.num_c + self.num_ip + email, 0]
        elif interval == self.MEDIUM_TERM:
            return self.rkc_medium[self.num_t + self.num_c + self.num_ip + email, 0]
        elif interval == self.LONG_TERM:
            return self.rkc_long[self.num_t + self.num_c + self.num_ip + email, 0]
