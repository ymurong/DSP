...
if __name__ == '__main__':
    ...
    predict_fn_rf = lambda x: rf0.predict_proba(x).astype(float)

    explainer = lime.lime_tabular.LimeTabularExplainer(X_train.values, feature_names=input_features,
                                                       mode='classification',
                                                       class_names=['NotFraud', 'Fraud'], kernel_width=5)
    dump_model(explainer)

    transaction_sample = X_test.sample()
    exp = explainer.explain_instance(transaction_sample.values[0], predict_fn_rf, num_features=100)

    for feature in exp.as_list():
        feature_name = getFeatureName(feature[0].split(" "))
        score = feature[1]
        explainable_group = getExplainableGroup(feature_name)
        explanability_scores[explainable_group] += score

    json_formatted_str = json.dumps(explanability_scores, indent=2)
